...
export default function PropertyMap({
  ...
  // Update clusters when properties, zoom, or bounds change
  useEffect(() => {
    if (!map || !clustererRef.current || !currentBounds) return;

    if (clustering && properties.length > 0) {
      ...
      const individualClusters = properties.map(property => ({
  ...
  }, [properties, currentZoom, currentBounds, clustering, map]);
